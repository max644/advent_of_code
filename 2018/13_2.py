...
class Circuit:
	...
	def detectCollision(self):
		positions = [(cart.x, cart.y) for cart in self.carts if not cart.collided]
		...
		for position, count in positionsCount.items():
			if count > 1:
				...
				for cart in self.carts:
					if cart.x == position[0] and cart.y == position[1]:
						cart.collided = True
...
if __name__ == "__main__":
	...
	#print circuit
	idx = 1
	while len([cart for cart in circuit.carts if not cart.collided]) > 1:
		circuit.move()
		#print idx
		idx += 1
		#if idx % 100 == 0:
	...
	cart = [cart for cart in circuit.carts if not cart.collided][0]
	print "===> " + str(cart.x) + "," + str(cart.y)
	
# 96,25 is not the correct answer
# 55,49 is not the correct answer